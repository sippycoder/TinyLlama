...
def balanced_split(dim: int, num_experts: int):
    # if num_experts <= 0 or (num_experts == math.isqrt(num_experts) ** 2) or (num_experts & (num_experts - 1) != 0):
    #     raise ValueError(f"Number of experts must be a power of 2 or a perfect square but there are {num_experts} experts")
    
    if num_experts not in [1, 4, 16]:
        raise ValueError(f"Number of experts must be one of 4, 8 and 9 but there are {num_experts} experts")
...
class ShadowCloneConfig(PretrainedConfig):
    r"""
